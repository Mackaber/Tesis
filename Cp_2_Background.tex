...
\Repeat{The stop criterion has been met}{
    ...
    \If{$U(0,1) < min(1,exp(\Delta E\Delta \beta))$}{
        Set the temp. of $replica_j$ and $replica_{j+1}$ to $T_{j+1}$ and $T_j$, respectively\;
        $\text{swap}(replica_j,replica_{j+1})$ \tcp{Keeps replicas ordered by temp.}
    } 
}
\end{algorithm}
...
$R_t \gets P_t \cup Q_t$ \; \tcp{combine parent and offspring population}\;
$F \gets \text{fast-non-dominated-sort}(R_t)$\; \tcp{run the fast non dominates sort procedure and collect $F = (F_1,F_2,...)$ with the non-dominated fronts of $R_t$}\;
$P_{t+1} = 0$ and $i = 1$\;
\Repeat{$|P_{t+1}| + |F_i| \leq N$ \; \tcp{until the parent population is filled}}{
    $\text{crowding-distance-assigment}(F_i)$\; \tcp{calculate crowding-distance in $F_i$}\;
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{hv_graph.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{spread_metric.png}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{epsiolon.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{igd_comp_1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{igd_comp_2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{igd_comp_3.png}
